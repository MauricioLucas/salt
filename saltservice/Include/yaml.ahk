...
MsgBox % Yaml_Dump(yaml,othervar,"push")
...
Yaml_Add(new,"data.mydata.new1","value") ;parent keys will be created automatically if they don't exist
Yaml_Add(new,"data.mydata.new1","value1") ;parent keys will be created automatically if they don't exist
Yaml_Add(new,"data.mydata 1.subkey","value1")
Yaml_Add(new,"data.mydata.subkey","value2")
...
Yaml_Add(pdic,key1="",key2=""){
	...
	If (pdic){
		...
		If !Yaml_Exist(pdic,key1){
			...
			count++
			Yaml_Set(pdic,parent!="" ? (parent "." 0) : 0,count)
			Yaml_Set(pdic,parent!="" ? (parent "." count) : count,key)
		} else if (!recursive){
			count:=Yaml_Get(pdic,key1 "." 0)
			If (count=""){
				count=2
				Yaml_Set(pdic,key1 ".1",Yaml_Get(pdic,key1))
			} else
				count++
			Yaml_Set(pdic,key1 "." 0,count)
			Yaml_Set(pdic,key1 "." count,key2)
		}
		Yaml_Set(pdic,key1,Yaml_Get(pdic,key1)="" ? key2 : Yaml_Get(pdic,key1) "," key2)
		Return 0
	} else if _pdic
		Return -1
...
;~ 		DllCall(NumGet(NumGet(pDic+0)+68), "UInt",pDic) ;delete all keys
	...
	If FileExist(Yaml_File){
		...
		FileIndex++
		Yaml_Set(pdic,0,FileIndex)
		Yaml_Set(pdic,FileIndex,Yaml_File)
		Yaml_Set(pDic,Yaml_File,_pDic)
	...
	Loop,Parse,Yaml_File_,`n,`r
	{
		...
		if !create
			Key1:="",Key2:="",Item:=""
		If (MainItem0="" || (!create && !RegExMatch(A_LoopField,"^\s+\w+:") && !RegExMatch(A_LoopField,"^\s+"".+"":"))){
			If !RegExMatch(A_LoopField,"^(\w+):",MainItem)
				If !RegExMatch(A_LoopField,"^""(.+)"":",MainItem)
					Continue
			MainItem0:=MainItem1
			ItemCount++
			MainItem:=Yaml_Get(_pdic,"") . (Yaml_Get(_pdic,"") ? "," : "") . MainItem0
			Yaml_Set(_pdic,"",MainItem)
			Yaml_Set(_pdic,0,ItemCount)
			Yaml_Set(_pdic,ItemCount,MainItem0)
			MainItem=
			Loop 100
				MainItem%A_Index%=
			Continue
		...
		If (!create){
			Loop 3
				Key%A_Index%=
			RegExMatch(A_LoopField,"^(\s+)(\w+):\s?(.*)$",Key)
			If (Key2=""){
				Loop 3
					 Key%A_Index%=
			...
			count++
;~ 			MsgBox % item "=" Key2 "`n" Yaml_Get(_pdic,Item) "`n" count
			Yaml_Set(_pdic,Item . ".0",count)
			Yaml_Set(_pdic,Item . "." . count,key2)
			Yaml_Set(_pdic,Item,MainItem . (MainItem="" ? "" : ",") . Key2)
			Item.="." . key2
			...
			If RegExMatch(Key3,"^\s*\[.+\]\s*$"){
				...
				Yaml_Set(_pdic,Item,Key3)
				Loop,Parse,Key3,`,,%A_Space%
					Yaml_Set(_pdic,Item "." A_Index,A_LoopField),Yaml_Set(_pdic,Item ".0", A_Index)
			} else Yaml_Set(_pdic,Item,key3)
		} else 
			Yaml_Set(_pdic,Item,Yaml_Get(_pdic,Item) . A_LoopField)
		If RegExMatch(Key3,"^\s*""")
			create:=1
